...
- Comparison between datasets (optional)

Usage:
    uv run verify_parquet_integrity.py /Volumes/Lightroom/ebird_by_location
    uv run verify_parquet_integrity.py /path/to/dir1 /path/to/dir2 --compare
"""
...
def verify_directory(dir_path: Path, verbose: bool = False) -> dict | None:
    ...
    Returns dict with stats, or None if verification failed.
    """
    print(f"\nVerifying: {dir_path}")
    print("-" * 60)
    ...
    for pf in parquet_files:
        try:
            ...
            schema_result = con.execute(
                f"DESCRIBE SELECT * FROM read_parquet('{pf}')"
            ).fetchall()
    ...
    if schemas:
        ...
        if inconsistent:
            print(f"\n  ⚠️  Schema inconsistency in {len(inconsistent)} files")
            if verbose:
...
def compare_datasets(stats_list: list[dict]) -> bool:
    ...
    Returns True if all counts match the first dataset.
    """
    if len(stats_list) < 2:
        return True
    ...
    max_name_len = max(len(str(s["path"].name)) for s in stats_list)

    print(f"\n{'Dataset':<{max_name_len}}  {'Rows':>15}  {'Size (GB)':>10}  {'Files':>6}")
    print("-" * (max_name_len + 40))

    for stats in stats_list:
        name = str(stats["path"].name)
        rows = stats["rows"]
    ...
    if all_match:
        ...
        print("⚠️  Row counts differ between datasets!")
        for stats in stats_list:
            if stats["rows"] != reference_rows:
                diff = stats["rows"] - reference_rows
                sign = "+" if diff > 0 else ""
                print(f"   {stats['path'].name}: {sign}{diff:,} rows vs reference")
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify integrity of Parquet datasets",
...
Examples:
  ...
  # Verify multiple directories and compare
  uv run verify_parquet_integrity.py \\
      /Volumes/Lightroom/ebird_parquet \\
      /Volumes/Lightroom/ebird_by_location \\
      /Volumes/Lightroom/ebird_by_date \\
      /Volumes/Lightroom/ebird_by_taxon

  # Verbose output (show each file)
  uv run verify_parquet_integrity.py -v /Volumes/Lightroom/ebird_sampling
        """,
    )
    parser.add_argument(
        "directories",
        type=Path,
        nargs="+",
        help="Directories containing Parquet files to verify",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    for dir_path in args.directories:
        stats = verify_directory(dir_path, verbose=args.verbose)
        if stats is None:
            any_failed = True
        else:
