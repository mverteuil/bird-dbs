...
def sort(
    ...
    result = sort_chunked(input_dir, output_dir, sort_order, memory_limit, max_temp_size)
    click.echo(f"\nTotal rows: {result['total_rows']:,}")
...
@click.option(
    "--from-step",
    type=click.Choice(["convert", "sort", "partition", "density_report", "build", "verify"]),
    default=None,
...
def run_all(
    ...
    5. build - Build region packs
    6. verify - Verify output integrity
    """
    from ebd_pack_builder.pipeline import PipelineManager
    from ebd_pack_builder.steps.build_region_packs import build_region_packs
    from ebd_pack_builder.steps.convert_to_parquet import convert_tarball_to_parquet
    ...
    if "sort" in steps:
        if manager.should_skip("sort", force):
            ...
            try:
                result = sort_chunked(parquet_dir, sorted_dir, "location", memory_limit)
    ...
    if "build" in steps:
        if manager.should_skip("build", force):
            ...
            try:
                result = build_region_packs(partitioned_dir, pack_manifest, packs_dir)
